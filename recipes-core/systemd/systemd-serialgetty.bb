	...
	if [ ! -z "${SERIAL_CONSOLES}" ] ; then
		...
		for entry in $tmp ; do
			...
			ttydev=`echo $entry | sed -e 's/^[0-9]*\;//' -e 's/\;.*//'`
                        echo "CliveLau: baudrate(${baudrate})/ttydev(${ttydev}/default_baudrate(${default_baudrate}))"
			if [ "$baudrate" = "$default_baudrate" ] ; then
				# enable the service
                                echo "CliveLau: enable the service"
				ln -sf ${D}${systemd_unitdir}/system/serial-getty@.service \
				...
				# install custom service file for the non-default baudrate
                                echo "CliveLau: install custom service file for the non-default baudrate"
				...
				# enable the service
				ln -sf ${D}${systemd_unitdir}/system/serial-getty$baudrate@.service \
